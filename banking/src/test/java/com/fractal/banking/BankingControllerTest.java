...
class BankingControllerTest {
	...
	void testGetBanks() throws Exception {
		mockMvc.perform(get("/fractal/banks")).andExpect(status().isOk()).andExpect(view().name("/all-banks"))
	...
	void testTransactions() throws Exception {
		...
		mockMvc.perform(get("/fractal/banking/" + bankId + "/accounts/" + accountId + "/transactions"))
				.andExpect(status().isOk()).andExpect(view().name("/transactions")).andDo(MockMvcResultHandlers.print())
				.andExpect(
						content().string(org.hamcrest.Matchers.containsString("0ef942ea-d3ad-4f25-857b-4d4bb7f912d8")));
	}
	...
	void testCategorizedTransactions() throws Exception {
		...
		mockMvc.perform(get("/fractal/banking/categories/transactions?companyId=" + companyId))
				.andExpect(status().isOk()).andExpect(view().name("/detailed-transactions"))
				.andDo(MockMvcResultHandlers.print())
